...
// Звіт. Виконавці, які не виконали внутрішні документи.
...
if (isset($d['users']) && $d['users']) {
	...
	e('<div class="user-list">');
		...
		e('<div class="doc-header">');
			...
			e('<span class="header-login">Логін</span>');
			e('<span class="header-count">Кількість</span>');
		e('</div>');
		...
		foreach ($d['users'] as $docRow) {
			...
			e('<div class="user-block">');
				...
				e('<span class="login">'. $profileLink .'</span>');
				e('<span class="count">[ '. count($UsTemp->notExecutionIncomingDocuments) .' ]</span>');
			e('</div>');
		}
