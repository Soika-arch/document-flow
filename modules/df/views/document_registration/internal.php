...
// Вид сторінки реєстрації внутрішнього документа.
...
e('<div class="fm">');
	e('<form class="fm_ad-incoming-document" enctype="multipart/form-data"'.
		...
		e('<div class="label_block">');
			e('<label for="dInternalDate">Дата документа</label>');
			e('<input type="date" name="dInternalDate" value="'. date('Y-m-d') .'" required>');
		e('</div>');
		...
		if (isset($d['users']) && $d['users']) {
			e('<div class="label_block">');
				e('<label for="dUserInitiator">Ініціатор документа</label>');
				e('<select name="dUserInitiator" required>');
		...
		if (isset($d['terms']) && $d['terms']) {
			e('<div id="dControlTerm" class="label_block hide_by-d_control_type">');
				e('<label for="dControlTerm">Термін виконання</label>');
